...
async function upsertJpegPreview(filepath, jpeg) {
  const meta = await readExif(filepath);
  const tagName = meta['Z-JpgTagName'];

  if (!tagName) {
    throw new Error('could not find tag name');
  }

  log.info(`writing jpeg to ${tagName} tag`);

  // TODO handle `result.error`
  await log.timing(
    `writing jpeg preview to ${filepath}`,
    async () => await execWithHooks({ lockfile: filepath }, 'execute', [
      '-b',
      `-${tagName}<=${jpeg}`,
      filepath,
...
function open(receive, send) {
  ...
  receive('exiftool:set:preview', async (ev, { filepath, id, jpeg }) => {
    handle(id, async () => await upsertJpegPreview(filepath, jpeg));
  });
}
