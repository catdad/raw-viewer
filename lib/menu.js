...
function createMenu({ events, experiments = {}, disableAnalytics }) {
  const experimentsList = [
    ...
    },
    {
      ...
      checked: !!experiments.disableCache
    },
    {
      label: 'Disable sharp (slow)',
      type: 'checkbox',
      click: (item) => {
        config.setProp('experiments.disableSharp', !!item.checked);
        events.emit('reload');
      },
      checked: !!experiments.disableSharp
    }
  ];
  ...
  if (!is.prod) {
    experimentsList.push({ type: 'separator' }, { role: 'toggledevtools' });
  }
