...
const openOnce = (() => {
  ...
  const initOnce = async () => {
    if (first) {
      first = false;
      await log.timing('exiftool:boot', async () => {
        await exifTool.open();
      });
    }
  };
...
function reportAnalytics(data) {
  if (data.Model) {
    analytics.event('metadata', 'camera', data.Model);
  }

  if (data.LensID) {
    analytics.event('metadata', 'lens', data.LensID);
  }

  if (data.FocalLength) {
    analytics.event('metadata', 'focal-length', data.FocalLength);
  }

  if (data.ExposureTime) {
    analytics.event('metadata', 'shutter-speed', data.ExposureTime);
  }

  if (data.ISO) {
    analytics.event('metadata', 'iso', data.ISO);
  }
}
