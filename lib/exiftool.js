...
function open(receive, send) {
  ...
  receive('exiftool:read:meta', async (ev, { filepath, id }) => {
    handle(id, async () => {
  ...
  receive('exiftool:read:shortmeta', async (ev, { filepath, id }) => {
    handle(id, async () => await readShortMeta(filepath));
  });
  ...
  receive('exiftool:copy:meta', async (ev, { filepath, id, targetpath }) => {
    handle(id, async () => await copyMeta(filepath, targetpath));
  });
}
